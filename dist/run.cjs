...
function classify(title, structuredType, description) {
  ...
  const gm = title.match(GRAD_RE) ?? description?.slice(0, 4e3)?.match(GRAD_RE) ?? null;
  const grad_year = gm ? parseInt(gm[1], 10) : null;
  let sponsorship = "unknown";
  if (description) {
    const tail = description.length > 8e3 ? description.slice(-6e3) : description;
...
function epochToIso(v) {
  ...
  const d = new Date(v > 1e12 ? v : v * 1e3);
